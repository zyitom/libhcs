...
private:
    ...
    static bool match_product_version(
        ...
        constexpr uint16_t mc02_product_id = 0x0723;
        constexpr std::string_view agent_product = "HCS Agent v" libhcs_PROJECT_VERSION_STRING;
        constexpr std::string_view mc02_product = "HCS Slave v" libhcs_PROJECT_VERSION_STRING;
        ...
        if (!product_matches) {
            if (warn_only_when_mismatch) {
                logging::get_logger().warn(
                    ...
                    product_string, libhcs_PROJECT_VERSION_STRING);
                return true;
            }
