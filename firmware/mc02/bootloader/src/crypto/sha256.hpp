...
namespace libhcs::firmware::crypto {
...
// 直接从调用方缓冲区按 64 字节整块喂入: 逐字节经 ctx->data 中转会在正式哈希前
// 为每个输入字节付出一次 load/store/自增/分支的开销, 整镜像扫描时占主导。
// sha256_transform 只做逐字节读取, 因此无需对齐即可直接消费源缓冲区。
