...
App::App() {
    ...
    // The UART port objects and their DMA rings into D2 SRAM, next to the
    ...
    // see the comment on libhcs_mpu_config in main.h.
    libhcs_mpu_config();
    ...
    // The two RS-485 ports (enclosure UART2 / UART3). Both MX_*_Init functions
    // also run HAL_RS485Ex_Init, which is what turns CR3.DEM on. The pair costs
    // about 1.8 KB of the 32 KB D2 SRAM region -- see uart.hpp. Compiles out
    // when libhcs_APP_RS485_ENABLE is OFF, the same way IMU skips MX_SPI2_Init.
    MX_USART2_UART_Init();
    MX_USART3_UART_Init();
#endif
    MX_TIM1_Init();
    ...
    // Shared time base. Must follow usb::vendor.init(), whose tusb_rhport_init()
    // assigns GINTMSK wholesale and would clear the SOF enable set here.
    // A no-op unless libhcs_APP_TIME_SYNC.
    sync::sof_init();
...
[[noreturn]] void App::run() {
    while (true) {
        ...
        // Shared time base: refit the microframe-to-cycle-counter line, and
        // re-arm the SOF enable so the hook survives a controller that was
        // reinitialized behind us. Compiles to nothing unless
        // libhcs_APP_TIME_SYNC.
        //
        // Paced off TIM5 rather than off a loop counter. This loop runs tens of
        // thousands of times a second and its period moves with USB load, so an
        // "every N passes" schedule would quietly make the fit period a function
        // of traffic -- and re-arming SOFM that often is a read-modify-write on
        // a peripheral register for no reason. hpm_board gets its 1 kHz tick
        // from a driver this board does not have, hence the explicit edge.
        ...
        // CAN telemetry (libhcs_APP_CAN_DIAG builds only); paces itself off the
        ...
        // The usb->try_transmit() calls are interleaved on purpose: a batch
        // filled by CAN1 starts moving toward the endpoint before UART1 is even
        // looked at. The profiler charges USB work to kUsb and each source to
