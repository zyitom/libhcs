...
> **状态**：现行有效
> **相关文档**：[仓库根 AGENTS.md](../../AGENTS.md) · [BUILD_ENVIRONMENT.md](BUILD_ENVIRONMENT.md)（完整环境搭建） · [USB_OPTIMIZATION_LOG.md](USB_OPTIMIZATION_LOG.md)（USB 调优实录：做过什么、否掉了什么） · [SOF_TIMEBASE.md](SOF_TIMEBASE.md)（跨板 USB SOF 时间轴验证） · [CONTROL_TIMING.md](CONTROL_TIMING.md)（控制环需要哪些时间信息）
...
> EtherCAT 桥固件已于 2026-09-08 移出本仓库，归档在
> `~/Desktop/ethercat-archive-2026-09-08/`（含选型实测、延迟路线图与从站栈全部源码）。
...
## 多板方案选型：N 块 USB 板 [实测 2026-08-01]
...
协议定义在 `core/include/libhcs/protocol/vendor_control.hpp`（主机与固件共用），
...
USB 构建里该回调返回 `false`（进 discard 模式），即明确拒绝而不是静默丢弃。
...
验证工具：`host/examples/ep0_config_test.cpp`（一块 0x5322 板，不需要 CAN/UART 接线）。
...
`dual_board_test contend 15000 20000 2 4 6 8`，`HCS_LATENCY_GAP_US=1000`，
...
`usb_packet_rate split3 0 10`，单管道（当时用 `libhcs_SPLIT_CAN_ENDPOINT=0` 关掉第二对
...
> `libhcs_SPLIT_CAN_ENDPOINT`、EP0 的 `0x46 kSetEndpointMode`、主机侧
> `enable_priority_can_channel` 全部删除），现在只有一对 `0x01`/`0x81`。本节的机制推理
> 仍然成立，取舍结论已被「分端点已默认关闭」一节取代。

**同一份交付配置，只切 `libhcs_SPLIT_CAN_ENDPOINT` 一个开关：**
...
（当时的 `libhcs_COPY_THEN_ARM`，vendor.cpp），turnaround 2.22 -> 1.27 us，**包率一模一样**：
...
负载下也几乎不动——所以它削掉的是约一半，不是全部。** 开关 `libhcs_SPLIT_CAN_ENDPOINT`
...
`dual_board_test` 现在支持 `HCS_LATENCY_HIST=1`，按 2 us 分桶打印分布（桶宽远小于
...
SDK 编译进去的 `libhcs_PROJECT_VERSION_STRING`，**正常路径下旧 SDK 已经被挡住了**。
...
| | packets/s |
|---|---|
| `libhcs_LATENCY_PROBE=ON` | 63223 / 63577 / 63200 |
| `libhcs_LATENCY_PROBE=OFF` | 63650 / 63629 / 63461 |

**约 0.4%，三对全部同向，但区间有重叠**——所以这是"至多约 0.5%"，不是一个精确值。
代价落在够不到的余量上（CAN 线速 19.8k f/s），而替代方案 `-Dlibhcs_CAN_DIAG=ON`
会污染 `kUart0` 上行流、让 `dual_board_test uart` 必然 FAIL（2026-09-04 为此烧了六次
板、追了一个不存在的 UART 回归）——所以 **2026-09-05 起埋点恒开，`libhcs_LATENCY_PROBE`
...
本机也没装 OpenOCD。**不要因此认为"看不到现场"**——带内通道已经入库，都是走 USB vendor
端点、编成 UART0 上行帧：
...
两个开关都默认 OFF：`CAN_DIAG` 会在 CAN 热路径上加计数器，而 `CAN_DIAG` 和
`SOF_DIAG` **都占用 `DataId::kUart0`**（本板的 UART1 数据口）把记录当 UART 上行帧
发出去（`diag/can_diag.cpp` 的 `write_uart(kUart0, ...)`）。
**两者共用这一条上行通道，不要同时打开**；`SOF_DIAG` 还会额外开 8 kHz 的 SOF 中断。
...
- `app/`、`bootloader/`：USB 固件两半。`app/src/xcore/`：core0 的跨核部分（环主机侧、
  次核装载、flash RPC 服务端），默认单核构建下整体编译为空——见本文末「遗留」一条。
