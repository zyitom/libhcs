...
/* libhcs LOCAL PATCH: EP2_Chain_Sel / EP3_T_Chain_Sel / EP3_R_Chain_Sel removed
...
void USBSS_Device_Endp_Init( )
{
    ...
    /* libhcs LOCAL PATCH: EP1 only -- the libhcs uplink/downlink pair. Leaving
    ...
    /* libhcs LOCAL PATCH: EP2 / EP3 arming removed with the endpoints.
...
{
    /* libhcs LOCAL PATCH: TIM12 exists only to time out SuperSpeed link
     * training and fall back to USB 2.0. With libhcs_USBSS_HS_FALLBACK == 0 we
    ...
    if( sta && !libhcs_USBSS_HS_FALLBACK )
    {
        return;
    }
