...
/* libhcs LOCAL PATCH: EP2 and EP3 are gone. libhcs uses exactly one bulk pair
...
/* libhcs LOCAL PATCH: EP1 is repurposed as the libhcs bulk pipe. The CH372
...
/* libhcs LOCAL PATCH: class-specific (DFU) control requests on EP0. Defined per
...
void USBSS_IRQHandler( void )              
{
    ...
    if(( status & USBSS_UDIF_SETUP ) && !( status & USBSS_UDIF_STATUS ))    // SETUP-DPH & ACK-TP
    {
        ...
        {
            /* libhcs LOCAL PATCH: dispatch class requests (DFU) instead of an
    ...
    else if( status & USBSS_UIF_TRANSFER )
    {
        ...
        if( ep_dir )
        {
            switch( endp_num )
            {
                ...
                case DEF_UEP1:
                    /* libhcs LOCAL PATCH: uplink IN complete (was EP1->EP2 loopback) */
                    usb_ss_ep1_in_complete();
                    break;

                /* libhcs LOCAL PATCH: EP2 / EP3 IN cases removed with the
        ...
        else 
        {
            switch( endp_num )
            {
                ...
                case DEF_UEP0:
                    /* libhcs LOCAL PATCH: hand the control-OUT payload (a
                ...
                case DEF_UEP1:
                    /* libhcs LOCAL PATCH: downlink OUT complete (was EP1->EP2 loopback) */
                    usb_ss_ep1_out_complete();
                    break;

                /* libhcs LOCAL PATCH: EP2 / EP3 OUT cases removed with the
                 * endpoints themselves; they now stall through the default. */
                default:
                   errflag = 0xFF;
                break;                  
